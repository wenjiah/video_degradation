...
if __name__ == "__main__":
    ...
    parser.add_argument('--shared_video_id', default=None)
    args = parser.parse_args()
    resolution = int(args.resolution)
    base_name = args.base_name
    shared_video_id = args.shared_video_id

    detect_result = []

    if shared_video_id == None:
        if base_name == "night-street":
            ...
            df.to_csv("../../Data/filtered/night-street/freq002_res"+str(resolution)+"_yolo/night-street-2017-12-17.csv", index=None)
            ...
            df = pd.DataFrame(detect_result, columns=['frame', 'object_name'])
            df.to_csv("../../Data/filtered/UA-DETRAC/freq1_res"+str(resolution)+"_yolo/UA-DETRAC-2020-09-19.csv", index=None)

    else:
        sample_path = "/z/wenjiah/video_degradation/data/sample_frames/UA-DETRAC_share/MVI_"+shared_video_id+"/"
        start_time = time.time()
        ...
        df = pd.DataFrame(detect_result, columns=['frame', 'object_name'])
        df.to_csv("../../Data/filtered/UA-DETRAC_share/"+shared_video_id+"_res"+str(resolution)+"_yolo/UA-DETRAC_share-2020-09-19.csv", index=None)
